...
function updategood(){
    ...
    //    return
    //}
    data+="&control="+JSON.stringify(controls);
    data+="&smstempid="+$("#SmsTempId").val();

    var ii=layer.msg("操作中……");
    $.getJSON("/index.php?r=goods/updategood&"+data,function(data){
        layer.close(ii);
        if(data.state!=0){
        ...
        layer.alert("修改成功",function(){
            ...
            //window.location.href="/index.php?r=goods/list-forsale";
        });
    });
...
function savegood() {
    ...
    var ii = layer.msg("操作中……");
    $.getJSON("/index.php?r=goods/updatebasenfo&" + data, function (data) {
        if (data.state != 0) {
            layer.alert(data.msg);
            layer.close(ii);
            return;
...
function addAttr(){
    var title="添加属性值";
    $.get("/index.php?r=goods/control","",function(msg){
        (new $.zui.ModalTrigger({custom: msg,title:title})).show();
    })
}
...
function openMark(){
    var title="地图标记";
    $.get("/index.php?r=admin-user/mark&name="+encodeURIComponent($("#address").val()),"",function(msg){
        //alert(msg);
        (new $.zui.ModalTrigger({custom: msg,title:title})).show();
    })
}
...
function query(){
    ...
    $.getJSON("/index.php?r=goods/listgood&id="+id,function(data){
...
function generateEboPricesWithData(data){
    ...
    listStr+="<th>商家pid（用于与商户交互，自动)</th></tr>";
    ...
    for(var index=0;index<priceType.length;index++){
        ...
        listStr+='<td><input id="'+index+'_-1" type="text" point="'+index+'_-1" value="'+pid+'" class="priceItem" style="width:100%;text-align:center;"></td></tr>';
    }
    listStr+="</table></div>";
...
function generateEboPrices(_self){
    ...
    for(var index=0;index<priceType.length;index++){
        ...
        listStr+='<td><input type="text" point="'+index+'_-1" class="priceItem" style="width:100%;" value="'+(eboPriceObj==null||index>=eboPriceObj.Items.length?"":(eboPriceObj.Items[index].ExtParams==undefined?"":eboPriceObj.Items[index].ExtParams))+'"></td></tr>';
    }
    listStr+="</table></div>";
