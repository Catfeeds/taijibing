...
function  salesVolume(){
   ...
   var sales2Val = Percentage3(datas2[0].same_time_total,datas2[0].use_total);
     ...
     if (sales2Val < 0) {
      ...
      $("#sales_of").text(Percentage3(datas2[0].use_total,datas2[0].user_num));
      $("#sales_of_year").text(Percentage3(datas2[0].year_use,1));
    }
    ...
    // var CustomerTypeunique = unique(CustomerType);
       ...
       for (var i = 0; i < CustomerType.length; i++) {
            ...
            if(CustomerType[i]==1){
                 ...
                 CustomerTypeNum[0]=Percentage2(CustomerTypeNum[0]+(map_datas[i].total_volume-map_datas[i].WaterRest),1)

            }else if(CustomerType[i]==2){

                CustomerTypeNum[1]=Percentage2(CustomerTypeNum[1]+(map_datas[i].total_volume-map_datas[i].WaterRest),1)

            }
            else if(CustomerType[i]==3){
                CustomerTypeNum[2]=Percentage2(CustomerTypeNum[2]+(map_datas[i].total_volume-map_datas[i].WaterRest),1)
            }
            else if(CustomerType[i]==99){
                CustomerTypeNum[3]=Percentage2(CustomerTypeNum[3]+(map_datas[i].total_volume-map_datas[i].WaterRest),1)
            }
        ...
        for(var i=0;i<CustomerTypeNum.length;i++){
                 $("#dataView tbody tr td:nth-child(2)").eq(i+1).text(CustomerTypeNum[i])
                    var num =    Percentage3(CustomerTypeNum[i],datas2[0].use_total)
            ...
            if(map_datas[j]==null||map_datas[j]==''){
             ...
             var mapProvinceLnum= map_datas[j].total_volume-map_datas[j].WaterRest
               if(mapProvinceUnique[i]==map_datas[j].Province){
                   mapProvinceL[i]=mapProvinceL[i]+mapProvinceLnum
            }
...
function mapProvinceLBar(mapceL ){
    var mapLBartotal = 0;
     for (var i = 0; i < mapceL.length; i++) {
         mapLBartotal=mapLBartotal+mapceL[i]
     }
         ...
         for (var j = 0; j < map_datas.length; j++) {
            ...
            var mapCityLnum= map_datas[j].total_volume-map_datas[j].WaterRest
              if(mapCityUnique[i]==map_datas[j].City){
                   mapCityL[i]=  mapCityL[i]+mapCityLnum    
            }
         }
        };
...
var mapCityLBaropi = [];
  ...
  for (var i = 0; i < mapProvinceL.length; i++) {
        mapProvinceLBaropi.push( Percentage3(mapProvinceL[i],mapProvinceNump ))
   }

   for (var i = 0; i < mapCityL.length; i++) {
               mapCityLBaropi.push( Percentage3(mapCityL[i],mapProvinceNump ))
   }
   for (var j = 0; j < mapProvinceUnique.length; j++) {
 
      if(mapProvinceUnique[j] == null){
           mapProvinceUnique[j] ='其它'
      }
       var html = '<div class="progress" style="height:10px;    background-color: #1d1f23;"><span class="name" style="margin-top:-5px;left: 25px;">' + mapProvinceUnique[j] + '</span>' + '<div class="progress-bar"   role="progressbar" aria-valuenow="60" aria-valuemin="0" aria-valuemax="100" style="width: ' + mapProvinceLBaropi[j] + '%; background-color: '+mapProvinceUnique[j]+'">' + '</div>' 
               + '<span class="baifenbi" style="color: #fff; position: absolute;    right: 50px;right: 45px;margin-top: -5px;">' + mapProvinceLBaropi[j] + '%'+ '</span> </div>'
