(function($, window, document, undefined) {
	...
	function Paging(element, options) {
		...
		this.options = {
			...
			totalPage:Math.ceil(datas[0].total/$('#Jumpdisplay option:selected').val()),
	...
	Paging.prototype = {
		...
		creatHtml: function() {
			...
			var total = Math.ceil(datas[0].total/$('#Jumpdisplay option:selected').val());
			...
			content += "<span class='totalPages'> 当前显示第&nbsp;<span id='toNum'>1</span>&nbsp;页 </span>";
		...
		bindEvent: function() {
			...
			me.element.on('click', 'a', function() {
				...
				if (id == "prePage") {
					...
					if (me.options.pageNo >= Math.ceil(datas[0].total/$('#Jumpdisplay option:selected').val())) {
						me.options.pageNo = Math.ceil(datas[0].total/$('#Jumpdisplay option:selected').val())
			  ...
			  me.element.on('keyup afterpaste', '#Jumpnump', function(){
				this.value=this.value.replace(/\D/g,'')
         		this.max=Math.ceil(datas[0].total/$('#Jumpdisplay option:selected').val())
		    	}).on('change',function() {
		    	    var   total= Math.ceil(datas[0].total/$('#Jumpdisplay option:selected').val())
