...
function initQiniu(){
    $.getJSON('/index.php?r=goods/getqiniukey',function(data){
        ...
        if(data.state==0){
            ...
            initUploader(key,'image8');
            initUploader(key,'image9');
            initUploader(key,'image10');
            initUploader(key,'image11');
            initUploader(key,'image12');
...
function savegood(){
   ...
   var memberdiscounttype= $("#memberdiscounttype").val();
    ...
    data+="&endtime="+encodeURIComponent(endtime);
    //上面是添加的-----------
