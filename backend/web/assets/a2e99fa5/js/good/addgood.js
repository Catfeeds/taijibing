...
var data={
    ...
    $.post("/index.php?r=goods/savegood",data,function(data){
...
function query(){
    $("#categoryid").empty();
    $("#merchantid").empty();
    $("<option value='0'>请选择运营中心</option>").appendTo($("#categoryid")).trigger("chosen:updated");
    $("<option value='0'>请选择服务中心</option>").appendTo($("#merchantid")).trigger("chosen:updated");
   // $("#categoryid").chosen({no_results_text: "没有找到",disable_search_threshold: 10}); //初始化chosen
   $("#merchantid").chosen({no_results_text: "没有找到",disable_search_threshold: 10}); //初始化chosen
    for(var i=0; i<category.length;i++) {
        var item = category[i];
        $("<option value='" + item["id"] + "'>" + item["name"] + "</option>").appendTo($("#categoryid")).trigger("chosen:updated");
    }

    for(var i=0; i<merchant.length;i++) {
        var item = merchant[i];
        $("<option value='" + item["id"] + "'>" + item["name"] + "</option>").appendTo($("#merchantid")).trigger("chosen:updated");;
    }
...
function change1(no){
    ...
    //if(category_id) {
        if (category_id) {
            ...
            //  $('#category2_id' + no).html('<option value=''>选择二级分类</option>');
...
if (category2_id) {
    ...
    console.log(objCategory)
            $.get('./?r=goods/get-brand',objCategory , function (data) {
                 var data = eval('(' + data+ ')');
                 console.log(data)
                 if (data != '') {
                     var html = "<option value=''>选择品牌</option>";
...
function change2(no){
    ...
    console.log(objCategory)
        ...
        $.get('./?r=goods/get-goods',objCategory,function(data){
                    var data = eval('(' + data+ ')');
            console.log(data)
            if(data!=''){
...
function change3(no){
    ...
    if(category_id==2){//茶吧机默认选中其他
        ...
        if(goodsname){
            ...
            $.get('./?r=goods/get-volume',objCategory,function(data){
 var data = eval('(' + data+ ')');
