...
$.post('./?r=sales-api/sales-detail-line', {'time': '4'}, function(json) {
  ...
  if (json) {
    ...
    _mapDataA =notempty(_mapDataA);
    _mapDataA=unique(_mapDataA)

    _mapDatab=notempty(_mapDatab);
    _mapDatab=unique(_mapDatab);
    ...
    // console.log(_mapDataA)
    // console.log(_mapDataAnum)
          ...
          option.tooltip.formatter = function(data) {
            if (provinceData) {
              ...
              for (var i = 0; i < provinceData.length; i++) {
                while (provinceData[i] == data[1] + '省'||provinceData[i] == data[1]||provinceData[i] == data[1]+ '市') {
