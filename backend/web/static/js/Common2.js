...
var addressResolve = function (options,province,city,area) {
    ...
    addressInfo.provInfo.chosen({no_results_text: "没有找到",disable_search_threshold: 10,search_contains: true}); //初始化chosen
    addressInfo.cityInfo.chosen({no_results_text: "没有找到",disable_search_threshold: 10,search_contains: true});//初始化chosen
    addressInfo.areaInfo.chosen({no_results_text: "没有找到",disable_search_threshold: 10,search_contains: true});//初始化chosen
    };
    /*城市选择绑定方法*/
        ...
        this.agenty=function(){
              if (obj.agenty_data.length) {
                  ...
                  var agentyOpts = "";
                    ...
                    addressInfo.agentyInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
                    addressInfo.agentfInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
                }
        }
           ...
           var addressInfo = this;
             ...
             this.addFun=function(){
                   addressInfo.nameInfo.empty().append("<option value=>"+nameTitle+"</option>");
                        ...
                        addressInfo.nameInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
             }
             this.init = function () {
              addressInfo.addFun();
...
function devlistFu2(obj){
       if(obj.data.length){
           var nameTitle = $('#'+obj.name).text();
           var addressInfo = this;
             this.namebInfo = $("#" + obj.name);//select对象
             this.addFun2=function(){
                   addressInfo.namebInfo.empty().append("<option value=>"+nameTitle+"</option>");
                        var devOpts = "";
                         $.each(obj.data, function (index, item) {
                            if (item) {
                                if(item){
                        ...
                        addressInfo.namebInfo.append(devOpts);
                        addressInfo.namebInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
             }
             this.init = function () {
              addressInfo.addFun2();
                  ...
                  if(obj.where){
                  $('#'+obj.name)
                    addressInfo.namebInfo.val(obj.where);
                    addressInfo.namebInfo.trigger("chosen:updated");
                  }
  ...
  // console.log(obj)  
    ...
    this.waterfactory=function(){
       ...
       var waterOpts = "";
        ...
        addressInfo.waterfactoryInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
        addressInfo.waterbrandInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
        addressInfo.waternameInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
        addressInfo.water_volumeInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
        }
...
// 
    this.waterbrand=function(){
                   ...
                   addressInfoWater.waterbrandInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
                   addressInfoWater.waternameInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
                   addressInfoWater.water_volumeInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
    };
...
var addresEquipmente= function (obj) {
    ...
    var addressInfo = this;
        ...
        this.devbrand=function(){
                  addressInfo.devnameInfo.not(":first").empty();

                  // $('.firs  li:gt(0)').
                   // addressInfo.devnameInfo.append("<option value=>请选择型号</option>");
                   ...
                   var Opts = "";
                    ...
                    addressInfo.devbrandInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
                    addressInfo.devnameInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
        }
...
var addresEquipmente2= function (obj) {
     //检测用户传进来的参数是否合法
    if (!isValid(obj))
        return this;
         //默认参数
    var addressInfo = this;
        ...
        this.logic_typeInfo = $("#" + obj.logic_type);//水厂 select对象
        this.devbrandInfo = $("#" + obj.devbrand);//服务中心 select对象
        this.devnameInfo = $("#" + obj.devname);//运营中心select对象
        this.logic_type=function(){
             addressInfo.devbrandInfo.empty();
             addressInfo.devbrandInfo.append("<option value=>请选择商品品牌</option>");  
             addressInfo.devnameInfo.empty();
             addressInfo.devnameInfo.append("<option value=>请选择商品型号</option>"); 
        ...
        this.devbrand=function(){
                  ...
                  // $('.firs  li:gt(0)').
                   // addressInfo.devnameInfo.append("<option value=>请选择型号</option>");
                 var logic_type=addressInfo.logic_typeInfo.val()  
                 var dataOa = obj.devbrand_data;
                ...
                if(logic_type){
                     $.ajax
                       ({
                           ...
                           async: false,
                           type: 'get',
                           data:{'logic_type':logic_type},
                           url: './index.php?r=supplier/get-brands-goods',
                ...
                console.log(dataOa)
                   var Opts = "";
                    jQuery.each(dataOa, function (index, item) {
                        if (item) {
                            // console.log(item)
                            Opts += "<option value='" + item.BrandName + "'   data='" + item.BrandNo + "' >" + item.BrandName + "</option>";
                        }
                    });
            ...
            this.devname=function(){
             addressInfo.devnameInfo.empty();
             addressInfo.devnameInfo.append("<option value=>请选择商品型号</option>");  
             var _thisVal = $("#water_brands  option:selected").attr('data')

           
             var logic_type=addressInfo.logic_typeInfo.val()  
             var dataOa = obj.devname_data;
            if(logic_type) {
                     $.ajax
                       ({
                           cache: false,
    ...
    this.init = function () {
        ...
        addressInfo.logic_type();
        addressInfo.logic_typeInfo.bind("change", addressInfo.devbrand);
        addressInfo.devbrand();
        addressInfo.devbrandInfo.bind("change", addressInfo.devname);
         addressInfo.devname();
        ...
        $("#removerSub").bind('click',function(){
             addressInfo.devbrandInfo.empty().append("<option value=>请选择商品品牌</option>");
             addressInfo.devnameInfo.empty().append("<option value=>请选择商品型号 </option>");
            addressInfo.devbrand();
            addressInfo.devbrandInfo.trigger("chosen:updated");
            addressInfo.devnameInfo.trigger("chosen:updated");
          })
          ...
          var addressInfo = this;
             ...
             this.addtypeFun=function(){
                   ...
                   addressInfo.version_Info.empty().append("<option value=>请选择版本号</option>");
                        ...
                        addressInfo.type_Info.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
                        addressInfo.version_Info.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
             }
       ...
       var addressInfo = this;
        this.usetypeInfo = $("#" + obj.name);//select对象
         ...
         this.usetypeb=function () {
          ...
          addressInfo.usetypeInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
         }
       ...
       var addressInfo = this;
        this.customertypeInfo = $("#" + obj.name);//select对象
         ...
         this.customertype=function () {
           addressInfo.customertypeInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
         }
...
function dev_state(obj){
       var addressInfo = this;
        this.customerInfo = $("#" + obj.name);//select对象
          ...
          // console.log(addressInfo.customertypeInfo)
         this.customertype=function () {
            addressInfo.customerInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
         }
...
function dev_state2(obj){
 ...
 //          // console.log(addressInfo.customertypeInfo)
         this.customertype=function () {
            addressInfo.stateInfo.chosen({no_results_text: "没有找到",search_contains: true,disable_search_threshold: 10}); //初始化chosen
         }
