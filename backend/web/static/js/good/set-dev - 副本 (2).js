...
function savegood(){
  ...
  var datayyy = eval('(' + goodType + ')');
    ...
    //    json[ i] = use_type[i];
    //}
    //var use_types = JSON.stringify(json);
    var use_types = use_type.join(",");

//console.log(use_type.join(","));return;
...
var htmlInput = '<input type="number" name=""  class="dvCBsrmb" value="" placeholder="请输入金额" style="background: #141619;border-radius:5px"/ >  &nbsp;元押金';
...
console.log(data.code)
     var html = '<div class="listBs"><input type="checkbox" name="use_type" value='+data.code+'  id="state'+dvCBsIndex+'" class="state" / >'
        html+='<label for="state'+dvCBsIndex+'"></label>'     
        html+='<span> '+prnb+'&nbsp;元押金</span> <div class="Bgclose"><i class="fa fa-times" aria-hidden="true"></i> </div><div>'
			//window.location.reload();
           $("#dvCBs").append(html)
        ...
        if (agent_id) {
            ...
            $.get('./?r=logic-user/get-devbrand', {'province':province,'city':city,'area':area,'agent_id': agent_id}, function (data) {
                ...
                if (data != '') {
                   ...
                   $('#devbrand' + no).trigger("chosen:updated");
                    //清空后面的下拉框数据
